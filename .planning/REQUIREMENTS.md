...
- [ ] **MOD-04**: Skill classifies the artifact family before applying rules, and says which one it chose — *implementation shipped; live-session reliability UNVERIFIED. Measured twice. Pre-fix, the Phase 3 UAT ran 15 live sessions (9 check-mode, 6 write-mode): check mode conformed 9/9; one write-mode session cited three rule findings (`PF-2.17`, `PF-2.15`, `PF-1.17`) before naming any family, then stopped to await a "proceed" reply instead of drafting (03-UAT.md test 2, gap G-03-2). Plan 03-05 made the source-material ask non-blocking and added an explicit no-rule-ID-before-the-family-line instruction to `## Your task`. Post-fix, an independent 16-session write-mode re-check (sonnet-5 and opus-5, five fixtures) found 14 conformant and 2 naming no family at all — the targeted defect is genuinely fixed (the fixture that previously ended its turn now names the family and drafts in the same response, on both models), but the overall rate moved 5/6 to 14/16, which is not a measurable improvement. A 4-run control ruled out the harness preamble as the cause. Do not re-mark Complete from a SUMMARY's `requirements-completed` field. Closure condition: a write-mode conformance run naming the family in every session, or a disclosed decision to accept the residual. Tracked as WINDOWS.md entry 8.*
