...
- [x] **EX-02**: Reader gets before/after pairs covering each artifact family, with the after column citing real rule numbers — *implementation shipped; content-quality reliability UNVERIFIED. All four family pairs are authored and structurally enforced (`before-after-family-missing` requires each family heading to carry both a ✗ and a ✓ line; `before-after-citation-missing` requires an allocated `PF-`/`MC-` token per section; `check_undefined_id` and `check_unlisted_figure` already bound every cited token and every figure). Whether each after column demonstrates the rewrite its cited rule asks for, rather than restating the rule's own wording, is authored in 04-02-PLAN.md as a `verification: backstop` truth precisely because no checker in this stack performs that judgment. Do not re-mark Complete from a SUMMARY's `requirements-completed` field — that field records implementation, not verification. Closure condition: a human read of the four after columns against the rule bodies they cite.*
