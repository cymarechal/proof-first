...
- [ ] **CAT-10**: The `description` field triggers the skill reliably on presales writing requests, acting as an explicit trigger list — *implementation shipped; reliability MEASURED 2026-09-20 and the requirement is NOT met. All 14 phrasings in `evals/pressure-tests.md` were run live (`evals/trigger/RESULTS-trigger.md`): must-fire 9 of 9 fired, but must-not-fire 2 of 5 also fired — a kickoff slide deck and a pricing/sizing calculation, both outside PROJECT.md scope and SKILL.md Limits. It triggers reliably; it does not act as an explicit trigger list. Do not re-mark Complete from a SUMMARY's `requirements-completed` field — that field records implementation, not verification. Closure condition: narrow the description, regenerate the derivatives, re-author the scope hash, and re-run `python3 evals/trigger/run_trigger_test.py`. Tracked in WINDOWS.md (id 4 closed — the test has been run; the over-fire is its own entry).*
