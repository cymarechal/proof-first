...
The additive sweep specified later checks for a missing differentiator, baseline, or reframe. Each absence found raises a `GAP` marker through the same mechanism as an invented number, so a thin-input draft comes back heavily marked, which is the correct signal.
...
No sentence runs longer than 25 words, counted as words delimited by whitespace, providing one stated definition of length rather than an implied one.
...
**Replace with:** the actor named as the subject, or, if unknown, stated plainly rather than hidden behind a passive.
